...
SentencePiece で分割した日本語テキストから、bigram / trigram の簡易言語モデルを作って文章生成する実験用リポジトリです。現状は `wiki.txt` を入力にして学習し、`predict3.py` または HTTP サーバー経由で推論できます。
...
Python 3 系と `sentencepiece` が必要です。
